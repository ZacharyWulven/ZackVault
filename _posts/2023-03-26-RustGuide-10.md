...
### 捕获值的方式
* 与函数获得参数的三种方式一样
1. 取得所有权：FnOnce，捕获时将变量的所有权 move 到自己的作用域内
2. 可变借用：FnMut，捕获可变借用，并对其值进行修改
3. 不可变借用：Fn，捕获不可变借用
...
> 所有实现了 Fn 的闭包都实现了 FnMut，而所有实现了 FnMut 的闭包都实现了 FnOnce
{: .prompt-info }
...
> 最佳实践：当指定 Fn trait bound 之一时，首先用 Fn，基于闭包体力的情况，如果需要 FnOnce 或 FnMut，编译器会再告诉你
{: .prompt-info }
...
1. 实现 Iterator trait 需要你定义一个 Item 类型，它用于 next 方法的返回类型（即迭代器的返回元素的类型）
